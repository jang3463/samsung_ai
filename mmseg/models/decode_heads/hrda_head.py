...
class HRDAHead(BaseDecodeHead):
    ...
    def resize(self, input, scale_factor, size = None):
        return _resize(
            input=input,
            size = size,
    ...
    def forward(self, inputs):
        ...
        # up_lr_seg = self.resize(lr_seg, None, (hr_seg.shape[2], hr_seg.shape[3]))
        if torch.is_tensor(att):
            att = self.resize(att, hr_scale / lr_scale)
            # att = self.resize(att, None, (hr_seg.shape[2], hr_seg.shape[3]))
